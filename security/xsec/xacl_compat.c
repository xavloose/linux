...
#include <linux/xacl_compat.h>

#include <asm/uaccess.h>

int copy_x_arg_wrapper_compat(const char *buf, struct x_arg_wrapper *uwrap)
{
	struct x_arg_wrapper_compat uwrapcompat;

        if (copy_from_user(&uwrapcompat, buf, sizeof(uwrapcompat)))
                return -EFAULT;

        if ((uwrapcompat.version != XSEC_VERSION) ||
	    (uwrapcompat.size != sizeof(struct x_arg_compat)))
                return -EINVAL;
...
int copy_x_arg_compat(const struct x_arg __user *buf, struct x_arg *arg)
{
	struct x_arg_compat argcompat;
...
int copy_x_hash_struct_compat(struct x_hash_struct *hash, const struct x_hash_struct *userp)
{
	struct x_hash_struct_compat hash_compat;
...
size_t get_x_arg_wrapper_size_compat(void)
{
