...
extern struct path x_real_root;

static struct x_policy_state running_polstate;
struct x_policy_state *polstate = &running_polstate;
extern struct x_alloc_state *current_alloc_state;

extern char *x_shared_page[4];
DEFINE_RWLOCK(x_inode_lock);

static unsigned int x_status __read_only = X_STATUS_INIT;
...
{
	return (x_status & X_READY);
}

void x_enable_rbac_system(void)
{
	pax_open_kernel();
	x_status |= X_READY;
	pax_close_kernel();
...
int x_rbac_disable(void *unused)
{
	pax_open_kernel();
	x_status &= ~X_READY;
	pax_close_kernel();
...
dev_t x_get_dev_from_dentry(struct dentry *dentry)
{
	return __get_dev(dentry);
}

u64 x_get_ino_from_dentry(struct dentry *dentry)
{
	return __get_ino(dentry);
}

static char x_task_roletype_to_char(struct task_struct *task)
{
	switch (task->role->roletype &
		(X_ROLE_DEFAULT | X_ROLE_USER | X_ROLE_GROUP |
		 X_ROLE_SPECIAL)) {
	case X_ROLE_DEFAULT:
		return 'D';
	case X_ROLE_USER:
...
{
	return x_task_roletype_to_char(current);
}

int
x_acl_tpe_check(void)
{
	if (unlikely(!(x_status & X_READY)))
		return 0;
	if (current->role->roletype & X_ROLE_TPE)
		return 1;
	else
		return 0;
}
...
int
x_streq(const char *a, const char *b, const unsigned int lena, const unsigned int lenb)
{
	if (likely(lena != lenb))
		return 0;
...
char *
x_to_filename_rbac(const struct dentry *dentry, const struct vfsmount *mnt)
{
	char *ret;
	read_seqlock_excl(&mount_lock);
	write_seqlock(&rename_lock);
...
static char *
x_to_proc_filename_rbac(const struct dentry *dentry, const struct vfsmount *mnt)
{
	char *ret;
	char *buf;
	int buflen;
...
char *
x_to_filename_nolock(const struct dentry *dentry, const struct vfsmount *mnt)
{
...
char *
x_to_filename(const struct dentry *dentry, const struct vfsmount *mnt)
{
...
char *
x_to_filename1(const struct dentry *dentry, const struct vfsmount *mnt)
{
...
char *
x_to_filename2(const struct dentry *dentry, const struct vfsmount *mnt)
{
...
char *
x_to_filename3(const struct dentry *dentry, const struct vfsmount *mnt)
{
...
__u32
to_x_audit(const __u32 reqmode)
{
	...
	return (((reqmode & ~X_AUDITS) << 10) | ((reqmode & X_WRITE) ? X_AUDIT_APPEND : 0));
}

struct acl_role_label *
__lookup_acl_role_label(const struct x_policy_state *state, const struct task_struct *task, const uid_t uid,
		      const gid_t gid)
{
	unsigned int index = x_rhash(uid, X_ROLE_USER, state->acl_role_set.r_size);
	...
	while (match) {
		if ((match->roletype & (X_ROLE_DOMAIN | X_ROLE_USER)) == (X_ROLE_DOMAIN | X_ROLE_USER)) {
...
found:
	if (match == NULL) {
	      try_group:
		index = x_rhash(gid, X_ROLE_GROUP, state->acl_role_set.r_size);
		match = state->acl_role_set.r_hash[index];

		while (match) {
			if ((match->roletype & (X_ROLE_DOMAIN | X_ROLE_GROUP)) == (X_ROLE_DOMAIN | X_ROLE_GROUP)) {
				for (x = 0; x < match->domain_child_num; x++) {
					if (match->domain_children[x] == gid)
						goto found2;
				}
			} else if (match->uidgid == gid && match->roletype & X_ROLE_GROUP)
				break;
			match = match->next;
		}
found2:
...
{
	unsigned int index = x_fhash(ino, dev, role->subj_hash_size);
...
{
	unsigned int index = x_fhash(ino, dev, subj->obj_hash_size);
...
struct name_entry *
__lookup_name_entry(const struct x_policy_state *state, const char *name)
{
	...
	while (match && (match->key != key || !x_streq(match->name, name, match->len, len)))
		match = match->next;
...
{
	unsigned int index = x_fhash(ino, dev, running_polstate.inodev_set.i_size);
...
void
__insert_inodev_entry(const struct x_policy_state *state, struct inodev_entry *entry)
{
	unsigned int index = x_fhash(entry->nentry->inode, entry->nentry->device,
...
{
	unsigned int index =
	    x_fhash(obj->inode, obj->device, subj->obj_hash_size);
...
{
	unsigned int index = x_fhash(obj->inode, obj->device, role->subj_hash_size);
	...
	const struct dentry *dentry, const struct vfsmount *mnt, char **path)
{
	...
	if (*path == NULL)
		*path = x_to_filename_nolock(dentry, mnt);
	    ...
	    const struct acl_subject_label *subj, char **path, const int checkglob)
{
	...
	*/
	if (orig_dentry == curr_dentry && newglob != X_CREATE_GLOB)
		newglob = X_NO_GLOB;
	      ...
	      const struct acl_subject_label *subj, char *path, const int checkglob)
{
	...
	if (unlikely((mnt == shm_mnt && inode->i_nlink == 0) || mnt == pipe_mnt ||
	    ...
	    IS_PRIVATE(inode))) {
		retval = (subj->mode & X_SHMEXEC) ? fakefs_obj_rwx : fakefs_obj_rw;
		goto out;
	}

	for (;;) {
		if (dentry == x_real_root.dentry && mnt == x_real_root.mnt)
			break;
	...
	if (retval == NULL)
		retval = full_lookup(l_dentry, l_mnt, x_real_root.dentry, subj, &path, checkglob);
out:
	write_sequnlock(&rename_lock);
	read_sequnlock_excl(&mount_lock);
...
{
	char *path = NULL;
	return __chk_obj_label(l_dentry, l_mnt, subj, path, X_REG_GLOB);
}
...
{
	char *path = NULL;
	return __chk_obj_label(l_dentry, l_mnt, subj, path, X_NO_GLOB);
}
...
{
	return __chk_obj_label(l_dentry, l_mnt, subj, path, X_CREATE_GLOB);
}
...
static void
x_log_learn(const struct dentry *dentry, const struct vfsmount *mnt, const __u32 mode)
{
	struct task_struct *task = current;
	const struct cred *cred = current_cred();

	security_learn(X_LEARN_AUDIT_MSG, task->role->rolename, task->role->roletype,
		       X_GLOBAL_UID(cred->uid), X_GLOBAL_GID(cred->gid), task->exec_file ? x_to_filename1(task->exec_file->f_path.dentry,
		       task->exec_file->f_path.mnt) : task->acl->filename, task->acl->filename,
		       1UL, 1UL, x_to_filename(dentry, mnt), (unsigned long) mode, &task->signal->saved_ip);

	return;
}

static void
x_log_learn_uid_change(const kuid_t real, const kuid_t effective, const kuid_t fs)
{
	struct task_struct *task = current;
	const struct cred *cred = current_cred();

	security_learn(X_ID_LEARN_MSG, task->role->rolename, task->role->roletype,
		       X_GLOBAL_UID(cred->uid), X_GLOBAL_GID(cred->gid), task->exec_file ? x_to_filename1(task->exec_file->f_path.dentry,
		       task->exec_file->f_path.mnt) : task->acl->filename, task->acl->filename,
		       'u', X_GLOBAL_UID(real), X_GLOBAL_UID(effective), X_GLOBAL_UID(fs), &task->signal->saved_ip);

	return;
}

static void
x_log_learn_gid_change(const kgid_t real, const kgid_t effective, const kgid_t fs)
{
	struct task_struct *task = current;
	const struct cred *cred = current_cred();
	...
	security_learn(GR_ID_LEARN_MSG, task->role->rolename, task->role->roletype,
		       X_GLOBAL_UID(cred->uid), X_GLOBAL_GID(cred->gid), task->exec_file ? x_to_filename1(task->exec_file->f_path.dentry,
		       task->exec_file->f_path.mnt) : task->acl->filename, task->acl->filename,
		       'g', X_GLOBAL_GID(real), X_GLOBAL_GID(effective), X_GLOBAL_GID(fs), &task->signal->saved_ip);

	return;
}

static void
x_set_proc_res(struct task_struct *task)
{
	...
	if (proc->mode & (X_LEARN | X_INHERITLEARN))
		return;
...
struct acl_subject_label *__x_get_subject_for_task(const struct x_policy_state *state, struct task_struct *task, const char *filename, int fallback)
{
	char *tmpname;
	struct acl_subject_label *tmpsubj;
	struct file *filp;
	...
	*/
	...
	if (filename)
		...
		preempt_disable();
		tmpname = x_to_filename_rbac(filp->f_path.dentry, filp->f_path.mnt);
...
static struct acl_subject_label *x_get_subject_for_task(struct task_struct *task, const char *filename, int fallback)
{
	return __x_get_subject_for_task(&running_polstate, task, filename, fallback);
}

void __x_apply_subject_to_task(const struct x_policy_state *state, struct task_struct *task, struct acl_subject_label *subj)
{
	struct acl_object_label *obj;
	struct file *filp;
...
#ifdef CONFIG_XKERNSEC_RBAC_DEBUG
	printk(KERN_ALERT "x_set_acls for (%s:%d): role:%s, subject:%s\n", task->comm, task_pid_nr(task), task->role->rolename, task->acl->filename);
#endif
}

static void x_apply_subject_to_task(struct task_struct *task, struct acl_subject_label *subj)
{
	__x_apply_subject_to_task(&running_polstate, task, subj);
}

__u32
x_search_file(const struct dentry * dentry, const __u32 mode,
	       const struct vfsmount * mnt)
{
	...
	*/
	if (unlikely(currobj->mode & X_INIT_TRANSFER && retval & X_WRITE &&
		...
		if (task->role != current->role) {
			...
			rcu_read_lock();
			read_lock(&xsec_exec_file_lock);
			subj = x_get_subject_for_task(task, NULL, 1);
			x_apply_subject_to_task(task, subj);
			read_unlock(&grsec_exec_file_lock);
			rcu_read_unlock();
			x_log_noargs(X_DONT_AUDIT_GOOD, X_INIT_TRANSFER_MSG);
		}
	}

	if (unlikely
	    ((curracl->mode & (X_LEARN | X_INHERITLEARN)) && !(mode & X_NOPTRACE)
	     && (retval != (mode & ~(X_AUDITS | X_SUPPRESS))))) {
		__u32 new_mode = mode;

		new_mode &= ~(X_AUDITS | X_SUPPRESS);

		retval = new_mode;

		if (new_mode & X_EXEC && curracl->mode & X_INHERITLEARN)
			new_mode |= X_INHERIT;

		if (!(mode & X_NOLEARN))
			x_log_learn(dentry, mnt, new_mode);
	}

	return retval;
}

struct acl_object_label *x_get_create_object(const struct dentry *new_dentry,
					      ...
					      const struct vfsmount *mnt)
{
	...
	preempt_disable();
	path = x_to_filename_rbac(new_dentry, mnt);
...
__u32
x_check_create(const struct dentry * new_dentry, const struct dentry * parent,
		const struct vfsmount * mnt, const __u32 mode)
{
	...
	matchpo = x_get_create_object(new_dentry, parent, mnt);

	retval = matchpo->mode & mode;

	if ((retval != (mode & ~(X_AUDITS | X_SUPPRESS)))
	    && (current->acl->mode & (X_LEARN | X_INHERITLEARN))) {
		__u32 new_mode = mode;

		new_mode &= ~(X_AUDITS | X_SUPPRESS);

		x_log_learn(new_dentry, mnt, new_mode);
		return new_mode;
	}
...
__u32
x_check_link(const struct dentry * new_dentry,
	      ...
	      const struct dentry * old_dentry, const struct vfsmount * old_mnt)
{
	...
	__u32 needmode;
	__u32 checkmodes = X_FIND | X_APPEND | X_WRITE | X_EXEC | X_SETID | X_READ |
			   X_DELETE | X_INHERIT;

	if (unlikely(!(x_status & X_READY)))
		return (X_CREATE | X_LINK);

	obj = chk_obj_label(old_dentry, old_mnt, current->acl);
	oldmode = obj->mode;

	obj = x_get_create_object(new_dentry, parent_dentry, parent_mnt);
	newmode = obj->mode;
	...
	needmode = oldmode & (X_NOPTRACE | X_PTRACERD | X_INHERIT | X_AUDITS);
	...
	if ((newmode & (X_CREATE | X_LINK)) == (X_CREATE | X_LINK))
		return newmode;
bad:
	needmode = oldmode;
	if (is_privileged_binary(old_dentry))
	...
	if (current->acl->mode & (X_LEARN | X_INHERITLEARN)) {
		x_log_learn(old_dentry, old_mnt, needmode | X_CREATE | X_LINK);
		return (X_CREATE | X_LINK);
	} else if (newmode & X_SUPPRESS)
		return X_SUPPRESS;
	else
		return 0;
}
...
int
x_check_hidden_task(const struct task_struct *task)
{
	if (unlikely(!(x_status & X_READY)))
		return 0;

	if (!(task->acl->mode & X_PROCFIND) && !(current->acl->mode & X_VIEW))
		return 1;
...
int
x_check_protected_task(const struct task_struct *task)
{
	if (unlikely(!(x_status & X_READY) || !task))
		return 0;

	if ((task->acl->mode & X_PROTECTED) && !(current->acl->mode & X_KILL) &&
...
int
x_check_protected_task_fowner(struct pid *pid, enum pid_type type)
{
	struct task_struct *p;
	int ret = 0;

	if (unlikely(!(x_status & X_READY) || !pid))
		return ret;

	read_lock(&tasklist_lock);
	do_each_pid_task(pid, type, p) {
		if ((p->acl->mode & X_PROTECTED) && !(current->acl->mode & X_KILL) &&
		    p->acl != current->acl) {
			ret = 1;
			goto out;
		}
...
void
x_copy_label(struct task_struct *tsk)
{
...
extern int x_process_kernel_setuid_ban(struct user_struct *user);

int
x_check_user_change(kuid_t real, kuid_t effective, kuid_t fs)
{
	unsigned int i;
	__u16 num;
	uid_t *uidlist;
...
#if defined(CONFIG_XKERNSEC_KERN_LOCKOUT)
	...
	globalreal = X_GLOBAL_UID(real);

	user = find_user(make_kuid(&init_user_ns, globalreal));
	if (user == NULL)
		goto skipit;

	if (x_process_kernel_setuid_ban(user)) {
		/* for find_user */
		free_uid(user);
		return 1;
	}
...
skipit:
	...
	if (current->acl->mode & (X_LEARN | X_INHERITLEARN))
		x_log_learn_uid_change(real, effective, fs);
	...
	if (!uid_valid(effective)) {
		...
		globaleffective = X_GLOBAL_UID(effective);
	}
	if (!uid_valid(fs)) {
		fsok = 1;
		globalfs = (uid_t)-1;
	} else {
		globalfs = X_GLOBAL_UID(fs);
	}

	if (current->acl->user_trans_type & X_ID_ALLOW) {
		for (i = 0; i < num; i++) {
			curuid = uidlist[i];
	...
	} else if (current->acl->user_trans_type & X_ID_DENY) {
		for (i = 0; i < num; i++) {
			curuid = uidlist[i];
			if (globalreal == curuid)
				break;
	...
	if (realok && effectiveok && fsok)
		...
		x_log_int(X_DONT_AUDIT, X_USRCHANGE_ACL_MSG, realok ? (effectiveok ? (fsok ? 0 : globalfs) : globaleffective) : globalreal);
		return 1;
	}
}

int
x_check_group_change(kgid_t real, kgid_t effective, kgid_t fs)
{
	unsigned int i;
	__u16 num;
	gid_t *gidlist;
	...
	if (current->acl->mode & (X_LEARN | X_INHERITLEARN))
		x_log_learn_gid_change(real, effective, fs);
	...
	if (!gid_valid(real)) {
		...
		globalreal = X_GLOBAL_GID(real);
	}
	if (!gid_valid(effective)) {
		effectiveok = 1;
		globaleffective = (gid_t)-1;
	} else {
		globaleffective = X_GLOBAL_GID(effective);
	}
	if (!gid_valid(fs)) {
		fsok = 1;
		globalfs = (gid_t)-1;
	} else {
		globalfs = X_GLOBAL_GID(fs);
	}

	if (current->acl->group_trans_type & X_ID_ALLOW) {
		for (i = 0; i < num; i++) {
			curgid = gidlist[i];
	...
	} else if (current->acl->group_trans_type & X_ID_DENY) {
		for (i = 0; i < num; i++) {
			curgid = gidlist[i];
			if (globalreal == curgid)
				break;
	...
	if (realok && effectiveok && fsok)
		...
		x_log_int(X_DONT_AUDIT, X_GRPCHANGE_ACL_MSG, realok ? (effectiveok ? (fsok ? 0 : globalfs) : globaleffective) : globalreal);
		return 1;
	}
}
...
void
x_set_role_label(struct task_struct *task, const kuid_t kuid, const kgid_t kgid)
{
	...
	if (unlikely(!filp)) {
		...
		return;
	} else if (!task->role || !(task->role->roletype & X_ROLE_SPECIAL)) {
	...
	if (role && task->role != role &&
	    (((role->roletype & X_ROLE_USER) && !x_acl_is_capable(CAP_SETUID)) ||
	     ((role->roletype & X_ROLE_GROUP) && !x_acl_is_capable(CAP_SETGID))))
		return;
	...
	if (task->inherited) {
		...
		*/
		subj = x_get_subject_for_task(task, task->acl->filename, 0);
		if (subj) {
			obj = chk_obj_label(filp->f_path.dentry, filp->f_path.mnt, subj);
			if (!(obj->mode & X_INHERIT))
				subj = NULL;
		}
	...
	/* if we changed uid/gid, but result in the same role
	   ...
	   lose subject
	*/
	if (role != origrole || (!(task->acl->mode & X_INHERITLEARN) &&
				   (subj == task->acl)))
		task->acl = subj;
...
int
x_set_proc_label(const struct dentry *dentry, const struct vfsmount *mnt,
		  const int unsafe_flags)
{
	...
	*/
	rcu_read_lock();
	read_lock(&tasklist_lock);
	if (task->ptrace && task->parent && ((task->parent->role->roletype & X_ROLE_GOD) ||
	    (task->parent->acl->mode & X_POVERRIDE))) {
		read_unlock(&tasklist_lock);
		rcu_read_unlock();
		goto skip_check;
	}
	...
	if (unsafe_flags && !(task->acl->mode & X_POVERRIDE) && (task->acl != newacl) &&
	     !(task->role->roletype & X_ROLE_GOD) &&
	     !gr_search_file(dentry, X_PTRACERD, mnt) &&
	     !(task->acl->mode & (X_LEARN | X_INHERITLEARN))) {
		if (unsafe_flags & LSM_UNSAFE_SHARE)
			x_log_fs_generic(X_DONT_AUDIT, X_UNSAFESHARE_EXEC_ACL_MSG, dentry, mnt);
		else if (unsafe_flags & (LSM_UNSAFE_PTRACE_CAP | LSM_UNSAFE_PTRACE))
			x_log_fs_generic(X_DONT_AUDIT, X_PTRACE_EXEC_ACL_MSG, dentry, mnt);
		else
			x_log_fs_generic(X_DONT_AUDIT, X_NNP_EXEC_ACL_MSG, dentry, mnt);
		return -EACCES;
	}
...
skip_check:
	...
	retmode = obj->mode & (X_INHERIT | X_AUDIT_INHERIT);

	if (!(task->acl->mode & X_INHERITLEARN) &&
	    ((newacl->mode & X_LEARN) || !(retmode & X_INHERIT))) {
		if (obj->nested)
			task->acl = obj->nested;
		else
			task->acl = newacl;
		...
		task->inherited = 1;
		if (retmode & X_INHERIT && retmode & X_AUDIT_INHERIT)
			x_log_str_fs(X_DO_AUDIT, X_INHERIT_ACL_MSG, task->acl->filename, dentry, mnt);
	}
...
void
x_handle_delete(const u64 ino, const dev_t dev)
{
...
update_acl_obj_label(const u64 oldinode, const dev_t olddevice,
		     ...
		     struct acl_subject_label *subj)
{
	unsigned int index = x_fhash(oldinode, olddevice, subj->obj_hash_size);
	struct acl_object_label *match;
...
update_acl_subj_label(const u64 oldinode, const dev_t olddevice,
		      ...
		      struct acl_role_label *role)
{
	unsigned int index = xr_fhash(oldinode, olddevice, role->subj_hash_size);
	struct acl_subject_label *match;
...
update_inodev_entry(const u64 oldinode, const dev_t olddevice,
		    const u64 newinode, const dev_t newdevice)
{
	unsigned int index = x_fhash(oldinode, olddevice, running_polstate.inodev_set.i_size);
	struct inodev_entry *match;
...
void
x_handle_create(const struct dentry *dentry, const struct vfsmount *mnt)
{
...
void
x_handle_proc_create(const struct dentry *dentry, const struct inode *inode)
{
	struct name_entry *matchn;
	...
	preempt_disable();
	matchn = lookup_name_entry(x_to_proc_filename_rbac(dentry, init_pid_ns.proc_mnt));
...
void
x_handle_rename(struct inode *old_dir, struct inode *new_dir,
		 ...
		 struct vfsmount *mnt, const __u8 replace, unsigned int flags)
{
	...
	*/
	...
	preempt_disable();
	matchn = lookup_name_entry(x_to_filename_rbac(old_dentry, mnt));
	...
	*/
	...
	if (exchange)
		matchn2 = lookup_name_entry(x_to_filename_rbac(new_dentry, mnt));
...
#if defined(CONFIG_XKERNSEC_RESLOG) || !defined(CONFIG_XKERNSEC_NO_RBAC)
static const unsigned long res_learn_bumps[X_NLIMITS] = {
	[RLIMIT_CPU] = X_RLIM_CPU_BUMP,
	[RLIMIT_FSIZE] = X_RLIM_FSIZE_BUMP,
	[RLIMIT_DATA] = X_RLIM_DATA_BUMP,
	[RLIMIT_STACK] = X_RLIM_STACK_BUMP,
	[RLIMIT_CORE] = X_RLIM_CORE_BUMP,
	[RLIMIT_RSS] = X_RLIM_RSS_BUMP,
	[RLIMIT_NPROC] = X_RLIM_NPROC_BUMP,
	[RLIMIT_NOFILE] = X_RLIM_NOFILE_BUMP,
	[RLIMIT_MEMLOCK] = X_RLIM_MEMLOCK_BUMP,
	[RLIMIT_AS] = X_RLIM_AS_BUMP,
	[RLIMIT_LOCKS] = X_RLIM_LOCKS_BUMP,
	[RLIMIT_SIGPENDING] = X_RLIM_SIGPENDING_BUMP,
	[RLIMIT_MSGQUEUE] = X_RLIM_MSGQUEUE_BUMP,
	[RLIMIT_NICE] = X_RLIM_NICE_BUMP,
	[RLIMIT_RTPRIO] = X_RLIM_RTPRIO_BUMP,
	[RLIMIT_RTTIME] = X_RLIM_RTTIME_BUMP
};

void
x_learn_resource(const struct task_struct *task,
		  const int res, const unsigned long wanted, const int gt)
{
	...
	if (unlikely((x_status & X_READY) &&
		     task->acl && (task->acl->mode & (X_LEARN | X_INHERITLEARN))))
		goto skip_reslog;

	x_log_resource(task, res, wanted, gt);
skip_reslog:

	if (unlikely(!(x_status & X_READY) || !wanted || res >= X_NLIMITS))
		return;

	acl = task->acl;

	if (likely(!acl || !(acl->mode & (X_LEARN | X_INHERITLEARN)) ||
	...
	if (wanted >= acl->res[res].rlim_cur) {
		...
		security_learn(X_LEARN_AUDIT_MSG, task->role->rolename,
			       task->role->roletype, X_GLOBAL_UID(cred->uid), X_GLOBAL_GID(cred->gid), acl->filename,
...
EXPORT_SYMBOL_GPL(x_learn_resource);
#endif
...
pax_set_initial_flags(struct linux_binprm *bprm)
{
	...
	if (proc->pax_flags & X_PAX_DISABLE_PAGEEXEC)
		flags &= ~MF_PAX_PAGEEXEC;
	if (proc->pax_flags & X_PAX_DISABLE_SEGMEXEC)
		flags &= ~MF_PAX_SEGMEXEC;
	if (proc->pax_flags & X_PAX_DISABLE_RANDMMAP)
		flags &= ~MF_PAX_RANDMMAP;
	if (proc->pax_flags & X_PAX_DISABLE_EMUTRAMP)
		flags &= ~MF_PAX_EMUTRAMP;
	if (proc->pax_flags & X_PAX_DISABLE_MPROTECT)
		flags &= ~MF_PAX_MPROTECT;

	if (proc->pax_flags & X_PAX_ENABLE_PAGEEXEC)
		flags |= MF_PAX_PAGEEXEC;
	if (proc->pax_flags & X_PAX_ENABLE_SEGMEXEC)
		flags |= MF_PAX_SEGMEXEC;
	if (proc->pax_flags & X_PAX_ENABLE_RANDMMAP)
		flags |= MF_PAX_RANDMMAP;
	if (proc->pax_flags & X_PAX_ENABLE_EMUTRAMP)
		flags |= MF_PAX_EMUTRAMP;
	if (proc->pax_flags & X_PAX_ENABLE_MPROTECT)
		flags |= MF_PAX_MPROTECT;
...
int
x_handle_proc_ptrace(struct task_struct *task)
{
	struct file *filp;
...
#ifndef CONFIG_XKERNSEC_HARDEN_PTRACE
	if (unlikely(!(x_status & X_READY)))
		return 0;
#endif
	...
	if (!filp || (task_pid_nr(tmp) == 0 && ((xsec_enable_harden_ptrace && x_is_global_nonroot(current_uid()) && !(x_status & X_READY)) ||
	...
	retmode = x_search_file(filp->f_path.dentry, X_NOPTRACE, filp->f_path.mnt);
	read_unlock(&xsec_exec_file_lock);
	read_unlock(&tasklist_lock);

	if (retmode & X_NOPTRACE)
		return 1;

	if (!(current->acl->mode & X_POVERRIDE) && !(current->role->roletype & X_ROLE_GOD)
...
void task_xsec_rbac(struct seq_file *m, struct task_struct *p)
{
	if (unlikely(!(x_status & X_READY)))
		return;

	if (!(current->role->roletype & X_ROLE_GOD))
		return;

	seq_printf(m, "RBAC:\t%.64s:%c:%.950s\n",
			p->role->rolename, x_task_roletype_to_char(p),
			p->acl->filename);
}

int
x_handle_ptrace(struct task_struct *task, const long request)
{
	struct task_struct *tmp = task;
	struct task_struct *curtemp = current;
	__u32 retmode;
	...
	if (request == PTRACE_ATTACH || request == PTRACE_SEIZE) {
		...
		if (task_pid_nr(tmp) == 0 && ((xsec_enable_harden_ptrace && x_is_global_nonroot(current_uid()) && !(x_status & X_READY)) ||
					((x_status & X_READY)	&& !(current->acl->mode & X_RELAXPTRACE)))) {
			read_unlock(&tasklist_lock);
			x_log_ptrace(X_DONT_AUDIT, X_PTRACE_ACL_MSG, task);
			return 1;
		}
		read_unlock(&tasklist_lock);
	}
	...
	retmode = x_search_file(task->exec_file->f_path.dentry, X_PTRACERD | X_NOPTRACE, task->exec_file->f_path.mnt);
	read_unlock(&xsec_exec_file_lock);
	...
	} else if (!(current->acl->mode & X_POVERRIDE) &&
...
static int is_writable_mmap(const struct file *filp)
{
	...
	if (x_status & X_READY && !(task->acl->mode & X_OVERRIDE) &&
	    !task->is_writable && d_is_reg(dentry) && (mnt != shm_mnt || (inode->i_nlink > 0))) {
		obj = chk_obj_label(dentry, mnt, running_polstate.default_role->root_label);
		obj2 = chk_obj_label(dentry, mnt, task->role->root_label);
		if (unlikely((obj->mode & X_WRITE) || (obj2->mode & X_WRITE))) {
			x_log_fs_generic(X_DONT_AUDIT, X_WRITLIB_ACL_MSG, dentry, mnt);
			return 1;
		}
	}
	return 0;
...
int
x_acl_handle_mmap(const struct file *file, const unsigned long prot)
{
	__u32 mode;
	...
	mode =
	    x_search_file(file->f_path.dentry,
			   X_EXEC | X_AUDIT_EXEC | X_SUPPRESS,
			   file->f_path.mnt);

	if (!x_tpe_allow(file))
		return 0;

	if (unlikely(!(mode & X_EXEC) && !(mode & X_SUPPRESS))) {
		x_log_fs_rbac_generic(X_DONT_AUDIT, X_MMAP_ACL_MSG, file->f_path.dentry, file->f_path.mnt);
		return 0;
	} else if (unlikely(!(mode & X_EXEC))) {
		return 0;
	} else if (unlikely(mode & X_EXEC && mode & X_AUDIT_EXEC)) {
		x_log_fs_rbac_generic(X_DO_AUDIT, X_MMAP_ACL_MSG, file->f_path.dentry, file->f_path.mnt);
		return 1;
	}
...
int
x_acl_handle_mprotect(const struct file *file, const unsigned long prot)
{
	__u32 mode;
	...
	if (unlikely(!(mode & X_EXEC) && !(mode & X_SUPPRESS))) {
		x_log_fs_rbac_generic(X_DONT_AUDIT, X_MPROTECT_ACL_MSG, file->f_path.dentry, file->f_path.mnt);
		return 0;
	} else if (unlikely(!(mode & X_EXEC))) {
		return 0;
	} else if (unlikely(mode & X_EXEC && mode & X_AUDIT_EXEC)) {
		x_log_fs_rbac_generic(X_DO_AUDIT, X_MPROTECT_ACL_MSG, file->f_path.dentry, file->f_path.mnt);
		return 1;
	}
...
void
x_acl_handle_psacct(struct task_struct *task, const long code)
{
	...
	if (unlikely(!(x_status & X_READY) || !task->acl ||
		     !(task->acl->mode & X_PROCACCT)))
		return;
	...
	x_log_procacct(X_DO_AUDIT, X_ACL_PROCACCT_MSG, task, wday, whr, wmin, wsec, cday, chr, cmin, csec, code);

	return;
}

#ifdef CONFIG_TASKSTATS
int x_is_taskstats_denied(int pid)
{
	...
	*/
	...
	if (task) {
		...
		if (gr_is_global_nonroot(cred->uid) && !groups_search(cred->group_info, xsec_proc_gid))
			ret = -EACCES;
#endif
#endif
		if (x_status & X_READY) {
			if (!(task->acl->mode & X_VIEW))
				ret = -EACCES;
		}
	} else
		ret = -ENOENT;
...
*/
int x_acl_enable_at_secure(void)
{
	if (unlikely(!(x_status & X_READY)))
		return 0;

	if (current->acl->mode & X_ATSECURE)
		return 1;

	return 0;
}

int x_acl_handle_filldir(const struct file *file, const char *name, const unsigned int namelen, const u64 ino)
{
	struct task_struct *task = current;
	...
	if (task->acl->mode & (X_LEARN | X_INHERITLEARN))
		return 1;
	...
	*/
	...
	preempt_disable();
	path = d_real_path(dentry, mnt, per_cpu_ptr(x_shared_page[0], smp_processor_id()),
			   bufsize);
...
void gr_put_exec_file(struct task_struct *task)
{
	...
	write_lock(&xsec_exec_file_lock);
	filp = task->exec_file;
	task->exec_file = NULL;
	write_unlock(&xsec_exec_file_lock);
...
#ifdef CONFIG_NETFILTER_XT_MATCH_XADM_MODULE
EXPORT_SYMBOL_GPL(x_acl_is_enabled);
#endif
#ifdef CONFIG_SECURITY
EXPORT_SYMBOL_GPL(x_check_user_change);
EXPORT_SYMBOL_GPL(x_check_group_change);
#endif
