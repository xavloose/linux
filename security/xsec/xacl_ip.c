...
#define X_BIND			0x01
#define X_CONNECT		0x02
#define X_INVERT		0x04
#define X_BINDOVERRIDE		0x08
#define X_CONNECTOVERRIDE	0x10
#define X_SOCK_FAMILY		0x20

static const char * x_protocols[IPPROTO_MAX] = {
...
const char *
x_proto_to_name(unsigned char proto)
{
	return x_protocols[proto];
}

const char *
x_socktype_to_name(unsigned char type)
{
	return x_socktypes[type];
}

const char *
x_sockfamily_to_name(unsigned char family)
{
	return x_sockfamilies[family];
}

extern const struct net_proto_family __rcu *net_families[NPROTO] __read_mostly;

int
x_search_socket(const int domain, const int type, const int protocol)
{
	...
	if (curr->sock_families[domain / 32] & (1U << (domain % 32))) {
		...
		if (curr->mode & (X_LEARN | X_INHERITLEARN)) {
			__u32 fakeip = 0;
			security_learn(X_IP_LEARN_MSG, current->role->rolename,
				       current->role->roletype, X_GLOBAL_UID(cred->uid),
				       X_GLOBAL_GID(cred->gid), current->exec_file ?
				       x_to_filename(current->exec_file->f_path.dentry,
				       current->exec_file->f_path.mnt) :
				       curr->filename, curr->filename,
				       &fakeip, domain, 0, 0, X_SOCK_FAMILY,
	...
	if (curr->mode & (X_LEARN | X_INHERITLEARN)) {
		...
		if (type == SOCK_RAW || type == SOCK_PACKET) {
			...
			security_learn(X_IP_LEARN_MSG, current->role->rolename,
				       ...
				       curr->filename, curr->filename,
				       &fakeip, 0, type,
				       protocol, X_CONNECT, &current->signal->saved_ip);
			...
			security_learn(X_IP_LEARN_MSG, current->role->rolename,
				       ...
				       curr->filename, curr->filename,
				       &fakeip, 0, type,
				       protocol, X_BIND, &current->signal->saved_ip);
		}
...
exit_fail:
	if (domain == PF_INET)
		x_log_str3(X_DONT_AUDIT, X_SOCK_MSG, x_sockfamily_to_name(domain),
			    x_socktype_to_name(type), x_proto_to_name(protocol));
	else if (rcu_access_pointer(net_families[domain]) != NULL)
		x_log_str2_int(X_DONT_AUDIT, X_SOCK_NOINET_MSG, x_sockfamily_to_name(domain),
			    x_socktype_to_name(type), protocol);
...
static int
x_search_connectbind(const int full_mode, struct sock *sk,
		      struct sockaddr_in *addr, const int type)
{
	...
	unsigned long i;
	int ret;
	int mode = full_mode & (X_BIND | X_CONNECT);
	__u32 ip_addr = 0;
	__u32 our_addr;
	__u32 our_netmask;
	char *p;
	...
	if (unlikely(!x_acl_is_enabled() || sk->sk_family != PF_INET))
		return 0;
	...
	if ((full_mode & X_BINDOVERRIDE) && addr->sin_addr.s_addr == htonl(INADDR_ANY) && curr->inaddr_any_override != 0)
		addr->sin_addr.s_addr = curr->inaddr_any_override;
	if ((full_mode & X_CONNECT) && isk->inet_saddr == htonl(INADDR_ANY) && curr->inaddr_any_override != 0) {
...
denied:
	if (mode == X_BIND)
		x_log_int5_str2(X_DONT_AUDIT, X_BIND_ACL_MSG, &ip_addr, ip_port, x_socktype_to_name(type), x_proto_to_name(sk->sk_protocol));
	else if (mode == X_CONNECT)
		x_log_int5_str2(X_DONT_AUDIT, X_CONNECT_ACL_MSG, &ip_addr, ip_port, x_socktype_to_name(type), x_proto_to_name(sk->sk_protocol));

	return -EACCES;
}

int
x_search_connect(struct socket *sock, struct sockaddr_in *addr)
{
	/* always allow disconnection of dgram sockets with connect */
	if (addr->sin_family == AF_UNSPEC)
		return 0;
	return x_search_connectbind(X_CONNECT | X_CONNECTOVERRIDE, sock->sk, addr, sock->type);
}

int
x_search_bind(struct socket *sock, struct sockaddr_in *addr)
{
	return x_search_connectbind(X_BIND | X_BINDOVERRIDE, sock->sk, addr, sock->type);
}

int x_search_listen(struct socket *sock)
{
	...
	return x_search_connectbind(X_BIND | X_CONNECTOVERRIDE, sock->sk, &addr, sock->type);
}

int x_search_accept(struct socket *sock)
{
	struct sock *sk = sock->sk;
	struct sockaddr_in addr;
...
int
x_search_udp_sendmsg(struct sock *sk, struct sockaddr_in *addr)
{
	if (addr)
		return x_search_connectbind(X_CONNECT, sk, addr, SOCK_DGRAM);
	else {
		...
		return x_search_connectbind(X_CONNECT | X_CONNECTOVERRIDE, sk, &sin, SOCK_DGRAM);
	}
}

int
x_search_udp_recvmsg(struct sock *sk, const struct sk_buff *skb)
{
	struct sockaddr_in sin;
